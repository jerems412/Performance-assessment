...
class RhController extends Controller
{
    ...
    public function spaceRh(Request $request)
    {
    ...
    public function listerEmployes(Request $request)
    {
    ...
    public function modifierDomain(Request $request)
    {
        return response()->json([
            'user' => session('userConnect'),
            'domain' => session('domainConnect'),
    ...
    // ajouter un employe
    public function persist_employe(Request $request)
    {
        $employe = new Employe();
        ...
        $employe -> domain_id = session('employeConnect')->domain_id;
        ...
        $employe -> matricule = "EMP".date('dmYB')."".$employe->id;
        $employe -> save();
        //user
        $user = new User();
    ...
    // ajouter un employe
    public function bloquer_employe(Request $request,$id)
    {
        $employe = Employe::find($id);
        $employe -> statut = true;
        $employe -> save();
    ...
    // ajouter un employe
    public function debloquer_employe(Request $request,$id)
    {
        $employe = Employe::find($id);
        $employe -> statut = false;
        $employe -> save();
    ...
    public function update_employe(Request $request,$id)
    {
        ...
        ($request->emailProfessionnel)?$employe -> emailProfessionnel = $request -> emailProfessionnel: null;
        ...
        $employe -> save();
        //user
        $user = User::where('roleusertable_id',$employe->id);
        $user = User::find($user->id);
        ($request->emailProfessionnel)?$user -> login = $request -> emailProfessionnel:null;
        $user -> save();
        return response()->json(true);
    }
    ...
    {
        //domain
        $domain = Domain::find(session('domainConnect')->id);
        ...
        $domain -> save();
        $request -> session() -> put('domainConnect',$domain);
        return response()->json(true);
    }
